...
def pseudo_DIs(DIs, random_seed=None, Nref=None):
    ...
    Inds = np.random.randint(int(len(DIs)), size=sample_size)
    D = np.array(DIs)
...
#commented
  ...
  # Compute the absolute difference between ages
  age_diff = np.abs(EP_data[:, 1] - age)



  indx = np.argmin(age_diff)



#commented#  rlon,rlat = pt_rot([EP_data[indx,2][0][0], EP_data[indx,3][0][0],EP_data[indx,4][0][0]],[plat],[plon])
  rlon,rlat = pt_rot([EP_data[indx,2], EP_data[indx,3],EP_data[indx,4]],[plat],[plon])
  ...
  if output_type == 'rvgps':
    ...
    return EP_data[indx,2], EP_data[indx,3], EP_data[indx,4]
...
def running_mean_APWP (data, plon_label, plat_label, age_label, window_length, time_step, max_age, min_age, elong=False):
    ...
    for age in mean_pole_ages:
        ...
        if (len(plons) > 1) and (len(plats) > 1):
...
def get_pseudopoles (df,age_min,age_max,N_s,EP_data):
    ...
    # GET PSEUDO VGPS
    pseudo_vgps = get_pseudo_vgps(df_iter['plon'].values,df_iter['plat'].values,df_iter['K'].values,
                                  df_iter['N'].values,df_iter['plateID'].values,df_iter['age'].values,
    ...
    if EP_data is None:
        ...
        # ROTATE VGPs
        rVGPs = get_rotated_vgps(selec_vgps['plon'].values,selec_vgps['plat'].values,selec_vgps['age'].values,
                                 selec_vgps['plate_id'].values,EP_data,output_type='rvgps')
        df_rvgps = pd.DataFrame(rVGPs, columns = ['rlon', 'rlat'])
    ...
    if N_s == 'all':
        pseudopole = ipmag.fisher_mean(dec=df_vgps['rlon'].tolist(), 
                                       inc=df_vgps['rlat'].tolist())
    # else:
    #     if len(df_vgps) < N_s:
    ...
    #     equal_n_vgps = df_vgps.sample(n = (int(N_s)), replace = True)

    #     pseudopole = ipmag.fisher_mean(dec=equal_n_vgps['rlon'].tolist(), inc=equal_n_vgps['rlat'].tolist())

    else:
    # If not enough reference VGPs exist…
      if len(df_vgps) == 0:
        print("ERROR: No VGPs available in this age window.")
        return [np.nan, np.nan, 0, np.nan]

      if len(df_vgps) < N_s:
        print(f"N_ref ({len(df_vgps)}) < N_studied ({N_s}) — using all available VGPs.")
        equal_n_vgps = df_vgps.copy()   # do NOT sample
      else:
        equal_n_vgps = df_vgps.sample(n=int(N_s), replace=True)

      pseudopole = ipmag.fisher_mean(
        dec=equal_n_vgps['rlon'].tolist(),
        inc=equal_n_vgps['rlat'].tolist()
      )
